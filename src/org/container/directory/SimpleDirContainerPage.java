 ...
 */
public class SimpleDirContainerPage extends WizardPage implements
	...
	public void createControl(Composite parent) {
		...
		GridData gd__dirCombo = new GridData(SWT.FILL, SWT.CENTER, true, false,
				2, 1);
		...
		_dirBrowseButton.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER,
				false, false, 1, 1));
		...
		_extText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				2, 1));
	...
	private boolean isDirValid(String dir) {
		...
		return _proj.getProject().getLocation().makeAbsolute()
				.isPrefixOf(dirPath);
	}
	...
	public boolean finish() {
		if (!areExtsValid(getExtensionValue())) {
			...
			setErrorMessage(NLS.bind(Messages.DirErr, _proj.getProject()
					.getName()));
			return false;
		}
		return true;
